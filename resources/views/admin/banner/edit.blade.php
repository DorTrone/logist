...
@section('content')
    ...
    <form action="{{ route('admin.banners.update', $obj->id) }}" method="post" enctype="multipart/form-data">
        {{ method_field('PUT') }}
        @csrf
        ...
        <div class="row row-cols-2 row-cols-md-4 g-3 mb-3">
            @foreach(['image' => null, 'image_tm' => 'tm', 'image_ru' => 'ru', 'image_cn' => 'cn'] as $name => $lang)
                <div class="col">
                    ...
                    <div class="mb-2">
                        <img src="{{ $obj->getImage($lang, false) ?: asset('img/400x400.png') }}" alt="@lang('app.image')" class="img-fluid border">
                    </div>
                    <input type="file" accept="image/*" class="form-control @error($name) is-invalid @enderror" id="{{ $name }}" name="{{ $name }}" {{ $loop->first ? 'autofocus' : '' }}>
                    @error($name)
        ...
        <div class="row row-cols-2 row-cols-md-4 g-3 mb-3">
            @foreach(['image_2' => null, 'image_2_tm' => 'tm', 'image_2_ru' => 'ru', 'image_2_cn' => 'cn'] as $name => $lang)
                <div class="col">
                    ...
                    <div class="mb-2">
                        <img src="{{ $obj->getImage2($lang, false) ?: asset('img/400x400.png') }}" alt="@lang('app.image')" class="img-fluid border">
                    </div>
        ...
        <div class="row row-cols-2 row-cols-md-4 g-3 mb-3">
            <div class="col">
                ...
                <input type="datetime-local" class="form-control @error('datetime_start') is-invalid @enderror"
                       id="datetime_start" name="datetime_start" value="{{ $obj->datetime_start }}" required>
            ...
            <div class="col">
                ...
                <input type="datetime-local" class="form-control @error('datetime_end') is-invalid @enderror"
                       id="datetime_end" name="datetime_end" value="{{ $obj->datetime_end }}" required>
                @error('datetime_end')
                <div class="invalid-feedback">{{ $message }}</div>
                @enderror
            </div>
            ...
            <div class="col">
                ...
                <input type="number" min="1" class="form-control @error('sort_order') is-invalid @enderror"
                       id="sort_order" name="sort_order" value="{{ $obj->sort_order }}" required>
                @error('sort_order')
                <div class="invalid-feedback">{{ $message }}</div>
                @enderror
            </div>
